...
def _conv(x, num_filters, kernel_size, stride, in_training, dropout_rate, name):
    with tf.variable_scope(name):
        ...
        x = tf.layers.conv2d(x, num_filters, kernel_size, stride, padding="same",
                             name="conv", use_bias=False, kernel_initializer=tf.contrib.layers.xavier_initializer())
...
def _conv_block(x, num_filters, kernel_size, stride, in_training, dropout_rate, name):
    with tf.variable_scope(name):
        y = x
        y = _conv(y, 4 * num_filters, 1, 1,
                  in_training, dropout_rate, "conv_1")
        y = _conv(y,     num_filters, 3, 1,
                  in_training, dropout_rate, "conv_2")
...
def _dense_block(x, num_layers, num_filters, growth_rate, in_training, dropout_rate, name):
    with tf.variable_scope(name):
        for i in range(num_layers):
            x = _conv_block(x, growth_rate, 1, 1, in_training,
                            dropout_rate, "conv_block" + str(i))
...
def _transition_block(x, num_filters, compression, in_training, dropout_rate, name):
    with tf.variable_scope(name):
        ...
        x = tf.layers.average_pooling2d(
            x, 2, 2, padding="same", name="avg_pool")
        return x, num_filters
...
def densenet121(inputs, num_classes,
    ...
    with tf.variable_scope(name):
        ...
        net = tf.layers.batch_normalization(
            net, name="batch_norm", training=in_training)
